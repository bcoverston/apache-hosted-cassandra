...
public abstract class MemtableBufferAllocator extends MemtableAllocator
{
    ...
    public MemtableRowData.ReusableRow newReusableRow(int nowInSec)
    {
        return MemtableRowData.BufferRowData.createReusableRow(nowInSec);
    }
    ...
    private static class RowBufferAllocator extends RowDataBlock.Writer implements RowAllocator
    {
        ...
        {
            // We're ignoring the max live timestamp because we'll recompute it
            // on query (the one we get through this method is only valid at
