...
public class SSTableReversedIterator extends AbstractSSTableIterator
{
    ...
    private ReusablePartitionData createBuffer(int blocksCount)
    {
        ...
        return new ReusablePartitionData(metadata(), partitionKey(), DeletionTime.LIVE, columns(), nowInSec(), estimatedRowCount);
    }
    ...
    private class ReusablePartitionData extends AbstractPartitionData
    {
        ...
        {
            super(metadata, partitionKey, deletionTime, columns, initialRowCapacity, false, nowInSec);

            this.rowWriter = new Writer();
            this.markerWriter = new RangeTombstoneCollector();
        }
        ...
        private void populateFrom(Reader reader, Slice.Bound start, Slice.Bound end, Tester tester) throws IOException
        {
            ...
            {
                // If we no start but still an openMarker, this means we're indexed and it's coming from the previous block
                ...
                writeMarker(markerStart, reader.getAndClearOpenMarker());
            }
        ...
        private void writeMarker(Slice.Bound bound, DeletionTime dt)
        {
            ...
            markerWriter.writeMarkerDeletion(dt);
